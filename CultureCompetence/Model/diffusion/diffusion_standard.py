...
num_epochs_flowers = 8
# KID = Kernel Inception Distance, see related section
kid_image_size = 75
kid_diffusion_steps = 6
plot_diffusion_steps = 12
...
class DiffusionStandardModel(tf.keras.Model):
    ...
    def learn_on_custom_dataset(self, train_dataset, val_dataset, n_images = 100, plot_imgs = True, aug=False, save=True, get_pretrained=True, percent=0.0, lamp=False, culture=0, category=0, imb=0, model_selection=True, parify_batches_diffusion=0, base_path = './', onlymin=0, test_set=None): 
        ...
        if not get_pretrained:
            ...
            for j in range(num_epochs_flowers):
                seq = list(range(15))
                random.shuffle(seq)
                for i in seq:
                    flowers_dataset = prepare_dataset(f"train[{2*(i)}%:{2*(i+1)}%]", image_size=self.image_size)
                    val_flowers_dataset = prepare_dataset(f"test[{100-(1)*(i+1)}%:{100-(1)*(i)}]", image_size=self.image_size)
